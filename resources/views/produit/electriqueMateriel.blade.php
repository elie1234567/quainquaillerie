<x-app-layout>
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        ...
        <div class="container-fluid py-4 px-5">
            ...
            <div class="container">
                <h1 id="ewa" style="text-align: center;"> 📳matériels électroniques 📳</h1>
                <br><br><br>
                ...
                <div class="card-deck">
                    @foreach ($electrique as $value)
                    <div class="card" id="el">
                        <img class="card-img-top" style="height: 290px; width: 290px;" src="data:image/jpeg;base64,{{ base64_encode($value->imagee) }}" alt="Image electrique">
                        <div class="card-body">
                             <h5 class="card-title">{{ $value->idE }}</h5>
                            <h5 class="card-title">{{ $value->articlee }}</h5>
                            <p class="card-text">Quantité: {{ $value->quantitee }}</p>
                            <p class="card-text">Prix unitaire: {{ $value->prixee }}</p>

                            <button type="button" 
                                        class="btn btn-success btn-edit"
                                         data-id="<?php echo e($value->idE); ?>"
                                         data-name="<?php echo e($value->articlee); ?>"
                                         data-email="<?php echo e($value->quantitee); ?>"
                                        data-phone="<?php echo e($value->prixee); ?>"
                                        data-location="<?php echo e($value->imagee); ?>">
                                        commande
                            </button>
...
<div class="modal fade" id="editUserModal" tabindex="-1" role="dialog" aria-labelledby="editUserModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form id="editUserForm" action="{{ route('products.store') }}" method="POST">
                @csrf
